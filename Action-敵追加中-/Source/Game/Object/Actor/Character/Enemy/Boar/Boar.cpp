...
//方向判定ボーンと当たった場合
void Boar::HitFrame(int _FrameNum) {
...
//通常攻撃１段目共通処理
void Boar::NoormalAttack1(TagState _State) {
	//１フレーム前の状態と今のフレームの状態を比較
	if (m_State != m_PrevState) {
		//変更があった
		m_PrevState = m_State;
...
//通常攻撃２段目共通処理
void Boar::NoormalAttack2(TagState _State) {
	//１フレーム前の状態と今のフレームの状態を比較
	if (m_State != m_PrevState) {
		//変更があった
		m_PrevState = m_State;
...
//攻撃パターン管理
void Boar::AttackPatternManager() {
	switch (m_DirectNum) {
	case REAR:
	case REAR_END:
		m_State = REAR_ATTACK;
