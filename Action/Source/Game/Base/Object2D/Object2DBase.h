...
public:
	...
	virtual void Load(const char* _FilePath, int _AllNum, int _XNum, int _YNum, int _XSize = 32, int _YSize = 32);
	//画像のアニメーションを呼び出し
	//_ID			:再生させるアニメID
	//_ChangeTime	:何フレームで画像を進めるか
	//_IsLoop		:TRUE	=	ループ再生
	//				:FALSE	=	終端再生
	virtual void RequestAnime(int _ID, int _ChangeTime, bool _IsLoop = true);
	//アニメーション再生
	virtual void AnimeUpdate();
	//アニメーション停止
	virtual void AnimeStop();
	//毎フレーム呼び出す処理
	virtual void Step() {};
	//描画処理
	virtual void Draw();
