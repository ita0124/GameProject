...
namespace {
	...
	constexpr int	SKILL_POINTS_COUNT = 10;				//取得スキルポイント回復量
}

//コンストラクタ
DropSkillPoints::DropSkillPoints() {
	Init();
}
//デストラクタ
DropSkillPoints::~DropSkillPoints() {
...
//初期化処理
void DropSkillPoints::Init() {
	DropItemBase::Init();

	m_DropItemKinds = SKILL_POINTS;		//ドロップアイテム種類
}
//毎フレーム呼び出す処理
void DropSkillPoints::Step() {
...
//当たり判定後の処理(当たっている場合)
void DropSkillPoints::HitCalc(ObjectBase* _Owner) {
	if (_Owner->GetKinds() == ObjectBase::TagKinds::PLAYER) {
		if (!m_IsHit) {
			...
			SoundManager::Play(SoundManager::TagID::SE_SKILL_POINTS);
			//旋回半径
...
void DropSkillPoints::Death() {
	...
	//
	int SkillPoints = PointerPlayer->GetSkillPoints();
	//
	SkillPoints += SKILL_POINTS_COUNT;
	//
	PointerPlayer->SetSkillPoints(SkillPoints);
}
